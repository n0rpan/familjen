...
--
-- RETENTION POLICY:
-- Events accumulate quickly (every PR generates multiple events).
-- Recommended: Set up a cron job to delete events older than 90 days.
-- Example cleanup query:
--   DELETE FROM ci_events WHERE created_at < NOW() - INTERVAL '90 days';
...
CREATE TABLE IF NOT EXISTS ci_events (
  ...
  source TEXT DEFAULT 'production', -- production, staging, local (distinguishes environments)
  data JSONB DEFAULT '{}',
...
CREATE INDEX IF NOT EXISTS idx_ci_events_source ON ci_events(source);
...
COMMENT ON TABLE ci_events IS 'Stores CI/CD events from GitHub Actions for admin dashboard. Retention: 90 days recommended.';
COMMENT ON COLUMN ci_events.type IS 'Event type: pr_opened, review_started, review_completed, verdict, labels_applied';
COMMENT ON COLUMN ci_events.source IS 'Environment source: production, staging, local';
