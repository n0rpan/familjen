...
import type { AdminPageData } from '@/lib/data/server'
...
function formatPrice(price: string | undefined | null, t: ReturnType<typeof useLanguage>['t']): string {
...
function ModelSelector({
  ...
  useEffect(() => {
    const fetchModels = async () => {
      try {
        const url = visionOnly ? '/api/openrouter/models?vision=true' : '/api/openrouter/models'
  ...
  if (loading) {
    return (
      <div className="input animate-pulse" style={{ background: 'var(--sand)' }}>
              ...
              <div key={provider}>
                ...
                {providerModels.map((model) => (
                  <button
                    ...
                    style={{ background: model.id === value ? 'var(--sand)' : undefined }}
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs truncate" style={{ color: 'var(--muted)' }}>
                        {model.id} • {(model.context_length / 1000).toFixed(0)}K context
                      </div>
                    </div>
...
export function AdminPageContent({ initialData, currentUserId }: AdminPageContentProps) {
  const { t } = useLanguage()
  const router = useRouter()
  const supabase = useMemo(() => createClient(), [])

  // Initialize state - use initial counts from server, load full data on mount
  const [allowedEmails, setAllowedEmails] = useState<AllowedEmail[]>(initialData.allowedEmails)
  ...
  // Initial data gives us quick stats - derive initial counts from server data
  const initialStats = useMemo(() => ({
    totalHouseholds: initialData.households.length,
    totalMembers: initialData.households.reduce((sum, h) => sum + h.members.length, 0),
    totalChildren: initialData.households.reduce((sum, h) => sum + h.children.length, 0),
    householdAdmins: initialData.households.reduce(
  ...
  const syncCalendar = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showMessage(
          'success',
          t.admin.syncSuccess.replace('{added}', data.synced).replace('{deleted}', data.deleted)
        )
        if (data.unmatchedInvites) {
          setUnmatchedInvites(data.unmatchedInvites)
        }
        const statusRes = await fetch('/api/calendar/sync')
        if (statusRes.ok) setCalendarStatus(await statusRes.json())
      } else {
  ...
  const addEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        loadData()
        showMessage('success', inviteAsHouseholdAdmin ? t.admin.userAddedCanCreate : t.admin.userAdded)
      }
    } catch {
  ...
  // Count stats - use initial server data while loading, then switch to live data
  const totalHouseholds = loading ? initialStats.totalHouseholds : households.length
  const totalMembers = loading ? initialStats.totalMembers : households.reduce((sum, h) => sum + h.members.length, 0)
  const totalChildren = loading ? initialStats.totalChildren : households.reduce((sum, h) => sum + h.children.length, 0)
  const householdAdmins = loading
    ? initialStats.householdAdmins
    : households.reduce((sum, h) => sum + h.members.filter((m) => m.is_household_admin).length, 0)
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div
          ...
          style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
        >
          <div className="text-3xl font-bold" style={{ color: 'var(--color-honey)' }}>
            {totalHouseholds}
          </div>
        ...
        <div
          ...
          style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
        >
          <div className="text-3xl font-bold" style={{ color: 'var(--color-sky)' }}>
            {householdAdmins}
          </div>
          <div className="text-sm" style={{ color: 'var(--muted)' }}>
        ...
        <div
          ...
          style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
        >
          <div className="text-3xl font-bold" style={{ color: 'var(--color-sage)' }}>
            {totalMembers}
          </div>
          <div className="text-sm" style={{ color: 'var(--muted)' }}>
        ...
        <div
          ...
          style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
        >
          <div className="text-3xl font-bold" style={{ color: 'var(--color-coral)' }}>
            {totalChildren}
          </div>
          <div className="text-sm" style={{ color: 'var(--muted)' }}>
      ...
      >
        ...
        {/* Add user form */}
        <form onSubmit={addEmail} className="mb-6 pb-6" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="badge badge-honey text-xs">{t.admin.becomesHouseholdAdmin}</span>
          </label>
        </form>
        ...
        <div className="space-y-2">
          ...
          {allowedEmails.map((item) => {
            ...
            const userMember = userHousehold?.members.find(
                ...
                <div className="col-span-5 flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {item.is_admin && <span className="badge badge-coral text-xs">{t.admin.appAdmin}</span>}
        ...
        <div className="space-y-2">
          {households.length === 0 ? (
            ...
            households.map((household) => {
              ...
              return (
                <div key={household.id} className="rounded-xl overflow-hidden" style={{ background: 'var(--background)' }}>
                  <button
                    ...
                    <div className="px-4 pb-4 space-y-4" style={{ borderTop: '1px solid var(--border)' }}>
                            ...
                            {household.children.map((child) => (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div
                                    className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold"
                                    style={{ background: `var(--color-${child.color || 'sky'})`, color: 'white' }}
                                  >
                      ...
                      <div className="pt-4" style={{ borderTop: '1px solid var(--border)' }}>
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            onClick={() =>
                              toggleHouseholdIntegrations(household.id, !household.external_integrations_enabled)
                            }
                            disabled={saving}
                            className="relative inline-flex h-6 w-11 items-center rounded-full transition-colors"
                            style={{
                              background: household.external_integrations_enabled ? 'var(--color-sage)' : 'var(--sand)',
                            }}
                          >
                            <span
                      ...
                      {household.external_integrations_enabled && <IntegrationDebug householdId={household.id} />}
      ...
      <section
        ...
        style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
      >
        <button onClick={() => setShowAuditLog(!showAuditLog)} className="w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div
              className="w-10 h-10 rounded-xl flex items-center justify-center"
          ...
          <div className="mt-6 space-y-3">
            {auditLog.length === 0 ? (
              ...
              auditLog.map((entry) => {
                ...
                return (
                  <div key={entry.id} className="p-3 rounded-xl" style={{ background: 'var(--background)' }}>
                    ...
                    {entry.action === 'UPDATE' && entry.changes && Object.keys(entry.changes).length > 0 && (
                      <div className="text-xs space-y-1 mt-2 pt-2" style={{ borderTop: '1px solid var(--border)' }}>
                        {Object.entries(entry.changes).map(([field, change]) => (
                          <div key={field} className="flex items-center gap-2">
                            <span style={{ color: 'var(--muted)' }}>{field}:</span>
                            <span style={{ color: 'var(--color-coral)' }}>{String(change.old ?? '-')}</span>
                            <span style={{ color: 'var(--muted)' }}>→</span>
                            <span style={{ color: 'var(--color-sage)' }}>{String(change.new ?? '-')}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {entry.action === 'INSERT' && entry.new_data && (
                      ...
                      >
                        {(() => {
                          const data = entry.new_data as Record<string, string | number | boolean | null>
                          const parts: string[] = []
                          if (data.name) parts.push(`Navn: ${data.name}`)
                          if (data.date) parts.push(`Dato: ${data.date}`)
                          if (data.custom_meal) parts.push(String(data.custom_meal))
                    ...
                    {entry.action === 'DELETE' && entry.old_data && (
                      ...
                      >
                        {(() => {
                          const data = entry.old_data as Record<string, string | number | boolean | null>
                          const parts: string[] = []
      ...
      <SectionHeader
        ...
        title={t.admin.systemConfigTitle || 'Systemkonfigurasjon'}
        description={t.admin.systemConfigDesc || 'AI-modeller og kalenderintegrasjon'}
      ...
      >
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ background: calendarStatus?.connected ? 'var(--color-sage)' : 'var(--color-coral)' }}
                />
                <div>
                  <div className="font-medium" style={{ color: 'var(--foreground)' }}>
                    {calendarStatus?.connected ? t.admin.connected : t.admin.notConnected}
                  </div>
          ...
          <div className="flex gap-3">
            {!calendarStatus?.connected ? (
              <a href="/api/calendar/auth" className="btn btn-primary flex items-center gap-2">
                <svg
                  width="18"
                  height="18"
                  viewBox="0 0 24 24"
              ...
              <>
                <button onClick={syncCalendar} disabled={syncing} className="btn btn-primary flex items-center gap-2">
                  {syncing ? (
                    <>
                ...
                <a href="/api/calendar/auth" className="btn btn-secondary flex items-center gap-2">
                  {t.admin.reconnect}
                </a>
              </>
            )}
      ...
      {/* Security Info */}
      <div className="flex items-start gap-3 p-4 rounded-xl" style={{ background: 'rgba(139, 168, 136, 0.15)' }}>
        <div
