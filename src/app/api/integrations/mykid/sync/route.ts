...
export async function POST(request: Request) {
  try {
    ...
    // Check rate limit
    const rateLimitKey = createRateLimitKey(user.id, 'mykidSync')
    const rateLimit = await checkRateLimit(rateLimitKey, RATE_LIMITS.mykidSync)
    ...
    if (!integrations || integrations.length === 0) {
      return NextResponse.json(
        { error: 'No MyKid integrations found' },
        { status: 404 }
      )
    }
