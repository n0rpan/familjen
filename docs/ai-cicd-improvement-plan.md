...
The app has a demo mode that provides mock data without requiring authentication. We use this for UAT testing:

### What Demo UAT Verifies

1. **Demo endpoints return valid, realistic data**
   - `/api/demo/*` endpoints respond correctly
   - Data shapes match TypeScript interfaces
   - Dates, times, Norwegian text are realistic

2. **Demo data covers production patterns**
   - If migrations add new columns, demo data should include them
   - If API returns new fields, demo should return them too
   - **Flag if demo is "behind" production capabilities**

3. **Demo mode doesn't skip critical validations**
   - Rate limiting still works in demo
...
```typescript
// Tool: verify_demo_quality()
// Compares demo API responses against production schema expectations

interface DemoQualityResult {
  endpoints_tested: number
  issues: DemoIssue[]
  coverage: {
    // Does demo data cover all fields in the type?
    type_coverage: Record<string, number>  // e.g., { "Child": 100, "Pickup": 85 }
  }
}
...
interface DemoIssue {
  ...
  expected?: string
  actual?: string
  migration_ref?: string  // "Added in migration 20251220_add_allergies.sql"
}
```
...
```
⚠️ Demo data quality issue:
  Endpoint: /api/demo/children
  Issue: missing_field
  Field: allergies
  Expected: string[] (added in migration 20251220_add_allergies.sql)
  Actual: field not present in demo response

  → Demo mode is behind production. Update src/app/api/demo/children/route.ts
```
...
Migrations run on merge to main via Supabase. We can't auto-rollback (data loss risk), but we can:

### Pre-Merge (PR Phase)

1. **AI analyzes migration safety** (existing)
2. **Generate reverse migration SQL** (new)
   - For each migration, AI generates the rollback SQL
   - Stored as artifact: `.ai-reviews/rollback-migrations/`
3. **Flag non-reversible migrations**
   - `DELETE FROM` without backup
   - `DROP COLUMN` (data loss)
   - `ALTER COLUMN TYPE` (precision loss)
...
post-deploy-health:
  ...
  # Runs 2 minutes after deploy to let Vercel + Supabase sync
  ...
  steps:
    ...
    - name: Health check
      run: |
        ...
        curl -f https://familjen.eu/api/health || exit 1

    - name: Verify DB connectivity
      run: npx tsx scripts/verify-db-health.ts
      env:
    ...
    - name: Alert on failure
      ...
      run: |
        ...
        echo "Check the rollback SQL in the PR that was just merged."
...
1. **Find the rollback SQL** in PR artifacts (`.ai-reviews/rollback-migrations/`)
2. **Review the SQL** - AI generated, human verified
3. **Run in Supabase SQL Editor** - Never auto-execute
4. **Redeploy previous version** via Vercel dashboard

### Generated Rollback Example

```sql
-- Rollback for: 20251220_add_allergies.sql
-- Generated by AI, review before executing
-- ⚠️ WARNING: This will DROP the allergies column. Data will be lost.
...
-- Remove from household_members too
ALTER TABLE household_members DROP COLUMN IF EXISTS allergies;
